import type { PriorityBand, RecommendedAction, Signal } from '../types';
...
export function triageBucket(signal: Signal): RadarTriageBucket {
  ...
  if (action === 'RESEARCH_REQUIRED' && !signal.researchBrief) return 'DECIDE_NOW';
  if (action === 'CREATE_OPPORTUNITY' || action === 'VIDEO' || action === 'SHORT_POST' || action === 'SAVE') {
    return 'REVIEW';
  }
...
/** Auto-descartar ruido: acción nula y score bajo. */
