...
| Finalize denormalized envelope + ADMIN CREATE integrity | before CODE_COMPLETE | T-009-14e | inventory §F |

### Final authorization model (T-009-14e)

- **Existing resources:** denormalized immutable `organizationId` (+ `clientId` on subs) on `resource.data` — no parent get.
- **CLIENT CREATE:** token org + token clientId + path + request envelope must agree — no parent get.
- **ADMIN CREATE (subs):** request envelope + `get(clients/{clientId}).organizationId == tokenOrg()` — referential integrity only; not primary tenant auth.
- **Root client CREATE:** `organizationId` required; no duplicated `clientId`; no parent get.

Why: zero lookups cannot prove path `clientId` belongs to ADMIN org at CREATE time with the current path model.
