...
| **W4 — Controller** ◐ **PARTIAL** | 5 | `main.ts` UI-logic extraction; Stage-A → Stage-B seam inversion — **4 responsibilities extracted** (presentation state, toasts, modal dispatch, navigation rules), 5,138 → **5,041** lines, named component imports 28 → 11. **Stage B entered** (T-010-403 formally accepted @ `66d849b3`) · React owns normal shell + navigation · legacy islands hosted · T-010-404 pending bootstrap reduction | CR-1 cutover spine complete (#9/#18). T-010-403 E2E verification complete (11 Playwright scenarios). Remaining W4 work is T-010-404 `main.ts` bootstrap/composition reduction — not dual navigation authority |
