...
| A9 | Every material score result records `scoringVersion` | Versioning | ✅ Phase 1–3 — Domain + Signal persistence |
| A10 | Score explainability reconstructs factors, penalties, total | Explainability | ✅ Phase 1–3 — breakdown + factor snapshots on Signal/history |
...
| A12 | Scoring persistence alone cannot silently perform terminal DISCARD | Auto-discard | ✅ Phase 4 — cloud ingest + legacy writer stripped |
| A13 | Cloud and client paths use same canonical formula/version or explicitly versioned approved divergence | Parity | ✅ Phase 4 — `tests/scoringPhase4.test.ts` |
| A14 | Legacy `applyScoreToSignal` is not an active governed strategic scoring path | Legacy | ✅ Phase 4 — deprecated; zero src callers |
| A15 | Score history preserves material scoring transitions | History | ✅ Phase 3 — `scoreHistoryCore` + local store |
| A16 | Tenant mismatch at scoring persistence boundary is rejected | Tenant | ✅ Phase 3 — adapter + use case tests |
| A17 | AI remains advisory; deterministic score remains authoritative | SPEC-005 boundary | ✅ Phase 4 — analyze no longer writes score fields; gateway failure degraded |
